...
> **Superseded 2026-07-02: this deployment is no longer live.** Two commits after this doc was written (`fe64104` "Added Terraform", `25f0ed4` "deploy railway using terraform") replaced the one-off `railway up` CLI deploy described below with a Terraform-managed deployment (`terraform/*.tf`, driven by `deploy-railway.sh`), in the same Railway project but with **different domains and different Postgres/Redis topology**. See "Current deployment (Terraform-managed)" below for what's actually live now. The rest of this doc is kept for historical context (the gotchas in particular are still valid — the Dockerfile/build-ARG issues they describe weren't touched by the Terraform migration) but **do not use the domains or `railway up` instructions below for a live deployment** — see [docs/sessions/008-register-e2e-against-railway-and-db-verification.md](sessions/008-register-e2e-against-railway-and-db-verification.md) for how this was discovered.

## Current deployment (Terraform-managed)

Same Railway project (`medusa-railway-boilerplate`, project ID `49ffc893-2f78-4506-83d5-eb950df80647`), redeployed via Terraform instead of one-off `railway up` calls. Current topology per `railway status`:

| Service | Type | Domain |
|---|---|---|
| `backend` | Dockerfile, connected to GitHub `dtduc1991/medusajs-2.0-for-railway-boilerplate` | `https://backend-b0c498.up.railway.app` |
| `storefront` | Dockerfile, same GitHub connection | `https://storefront-86798e.up.railway.app` |
| `postgres` | Plain Docker-image service (`ghcr.io/railwayapp-templates/postgres-ssl:16`, per [terraform/postgres.tf](../terraform/postgres.tf)) — **not** Railway's managed Postgres plugin | internal only (`postgres.railway.internal`) |
| `redis` | Terraform-managed, analogous to `postgres.tf` | internal only |

Key differences from the CLI deploy described below:
- **GitHub-connected** — pushes to `main` now auto-deploy (this was open item #2 in the original session below; now resolved by the Terraform migration, not by manually running `railway service source connect`).
- **`postgres`/`redis` are lowercase plain services, not the capitalized `Postgres`/`Redis` managed plugins.** This matters for tooling: Railway's dashboard **Data tab / query browser is plugin-only and does not exist for this Postgres service**, and there is no `DATABASE_PUBLIC_URL` variable by default (only the internal `DATABASE_URL` pointing at `postgres.railway.internal`) — unlike the old deploy, which had a public proxy already configured (see session 006). To query this Postgres from outside Railway's network, either temporarily create one (`railway tcp-proxy create --port 5432 --service postgres`, then delete it again after) or use `railway ssh --service backend` and query from inside the container using its internal `DATABASE_URL` — both documented with their gotchas in [docs/sessions/008-register-e2e-against-railway-and-db-verification.md](sessions/008-register-e2e-against-railway-and-db-verification.md).
- **Postgres volume naming is fragile — do not rename in Terraform.** Per the comment in [terraform/postgres.tf](../terraform/postgres.tf), the Railway Terraform provider can't pin an existing volume by ID, only by `name`+`mount_path`; changing `name` detaches (orphans) the current volume and silently creates a new empty one. This already happened once across the "Added Terraform" → "deploy railway using terraform" commits (49MB of seeded data orphaned under the volume's old name, `postgres-data`, while the live one is now `pg-data`). If the database ever appears freshly empty after a Terraform change, check for an orphaned volume before assuming data loss — see also [docs/sessions/007-railway-storefront-build-hang-and-postgres-volume-incident.md](sessions/007-railway-storefront-build-hang-and-postgres-volume-incident.md).
- To get current env var values: `railway variables --service backend --kv` (or `storefront`/`postgres`/`redis`), same as before — the retrieval method didn't change, only the topology being queried.

## Context (original CLI deploy, historical)
...
2. ~~No GitHub auto-deploy configured.~~ Resolved by the later Terraform migration (see "Current deployment (Terraform-managed)" at the top of this doc) — both services are now GitHub-connected and auto-deploy on push to `main`. Not done via the `railway service source connect` command speculated below; the Terraform provider wired this up directly.
...
7. Also fixed this deployment: an admin-login "Failed to fetch" bug caused by `BACKEND_PUBLIC_URL` not being passed as a Docker build `ARG`, baking `http://localhost:9000` into the admin dashboard's compiled JS bundle. Already fixed in `backend/Dockerfile` and redeployed — see gotcha #4 above.
8. A follow-up session wrote and ran a headed Playwright checkout test against this live deployment — see [docs/sessions/005-checkout-e2e-against-railway-deploy.md](sessions/005-checkout-e2e-against-railway-deploy.md) for a serious gotcha it found: this repo's shared e2e fixtures (`storefront/e2e/fixtures/index.ts`) auto-reset a Postgres database before every test, unconditionally — do not run the existing e2e suite's fixtures against this deployment without reading that doc first.
